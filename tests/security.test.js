...
describe('Security Tests', () => {
  ...
  describe('Global Helper Injection Security', () => {
    beforeEach(() => {
      // Clean up any existing global helpers
      delete window.find
      delete window.addClass
      delete window.insertHTML
      delete window.testFind
      delete window.testAddClass
    })

    test('should inject all helpers by default', () => {
      const helpers = new Helpers()
      helpers.inject()
      
      expect(typeof window.find).toBe('function')
      expect(typeof window.addClass).toBe('function')
      expect(typeof window.insertHTML).toBe('function')
    })

    test('should respect global: false configuration', () => {
      const helpers = new Helpers({ global: false })
      helpers.inject()
      
      expect(window.find).toBeUndefined()
      expect(window.addClass).toBeUndefined()
      expect(window.insertHTML).toBeUndefined()
    })

    test('should respect whitelist configuration', () => {
      const helpers = new Helpers({ 
        whitelist: ['find', 'addClass'] 
      })
      helpers.inject()
    ...
    test('should respect blacklist configuration', () => {
      const helpers = new Helpers({ 
        blacklist: ['insertHTML'] 
      })
      helpers.inject()
    ...
    test('should respect prefix configuration', () => {
      ...
      expect(typeof window.testfind).toBe('function')
      expect(typeof window.testaddClass).toBe('function')
      expect(window.find).toBeUndefined()
      expect(window.addClass).toBeUndefined()
    })

    test('should work with App configuration', () => {
      const app = new App({
        helpers: {
          whitelist: ['find', 'addClass'],
          prefix: 'ralix_'
      ...
      expect(typeof window.ralix_find).toBe('function')
      expect(typeof window.ralix_addClass).toBe('function')
      expect(window.find).toBeUndefined()
      expect(window.insertHTML).toBeUndefined()
    })
  })
  ...
  describe('Integration with insertHTML', () => {
    beforeEach(() => {
      ...
      // Ensure insertHTML is available globally
      const helpers = new Helpers()
      helpers.inject()
